...
export default function QuoteForm() {
  ...
  const canSubmit = name.trim() && phoneDigits.length === 10;
      ...
      <div className="bg-dark-lighter border border-zinc-800 rounded-2xl shadow-2xl p-8 max-w-2xl mx-auto">
        <div className="flex flex-col items-center justify-center py-8">
          ...
          <p className="text-gray-400 text-sm text-center max-w-md mb-6">
            Thanks for contacting Express Auto Detail. We&apos;ll reach out shortly to confirm your service details and schedule your mobile detailing appointment in the Bucks County area.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div id="quote" className="bg-dark-lighter border border-zinc-800 rounded-2xl shadow-2xl max-w-2xl mx-auto overflow-hidden">
      ...
      <div className="p-6 lg:p-8 space-y-4">
        ...
        <div>
          <label className="block text-xs font-medium text-gray-400 mb-1">Tell us about your vehicle</label>
          <textarea
            value={details}
            onChange={(e) => setDetails(e.target.value)}
            placeholder="Year, make, model, condition, services you're interested in..."
            rows={3}
            className="w-full px-3.5 py-2.5 rounded-lg border border-white/10 bg-dark text-white outline-none transition text-sm focus:ring-2 focus:ring-primary/40 focus:border-primary placeholder-gray-600 resize-none"
          />
        </div>
